...
| `data/adjudication/adjudication_log_199_all_fields.csv` | Study-field assignment | Full adjudication audit log for all 995 controlled study-field assignments: 585 initial agreements (583 unchanged and two later U17 corrections), 410 OY rereviews (409 retained and one later C11/MALF official-record correction), represented by 583 `agreed_assignment`, 409 `third_party_external_rereview`, and three correction rows. |
...
| `data/adjudication/final_matrix_reapplication_check_tasks_60.csv` | Post-adjudication diagnostic task | Dou Xingwang's completed 300-task reapplication of the five frozen fields on 60 sampled studies, with source locators and excerpts; not an independent replication or accuracy estimate. |
...
| `data/adjudication/rong_external_interpretability_check_50.csv` | External interpretability check | Record-level judgments for Rong Zhoujie's fixed 50-study sample across four operational-presence fields; includes the pre-adjudication reference label, evidence locator, unresolved status, and binary comparison columns. |
